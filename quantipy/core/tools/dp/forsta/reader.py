"""Forsta survey platform integration for quantipy3.

This module provides functionality for reading and converting Forsta JSON survey data
and metadata into quantipy-compatible data structures, supporting complex survey
formats including grids, loops, and multi-choice variables.
"""
...
def quantipy_from_forsta(
    ...
    """Convert Forsta survey data and metadata to quantipy format.

    This function processes Forsta JSON survey data and metadata, converting them
    into quantipy-compatible structures with proper variable types, relationships,
    and metadata organization.

    Parameters
    ----------
    self : object
        The calling DataSet instance
    meta_json : dict | str
        Forsta metadata as dictionary or path to JSON file
    data_json : list | str
        Forsta data as list of records or path to JSON file
    verbose : bool, default False
        Include additional Forsta-specific metadata in output
    text_key : str, default 'en-GB'
        Language key for text labels

    Returns
    -------
    tuple[dict, pd.DataFrame]
        Quantipy metadata dictionary and pandas DataFrame with survey data

    Notes
    -----
    Supports complex Forsta survey structures including:
    - Single and multiple choice questions
    - Grid/array questions with various subtypes
    - Ranking questions with numeric ordering
    - Loop structures for dynamic questionnaires
    - Multi-language text handling
    - Complex grid relationships and hierarchies
    """
    types_translations = {
        'numeric': 'float',
        'text': 'string',
    ...
    to_qp_format = {}
    to_forsta_format = {}
    def create_subvar_meta(parsed_meta: dict, subvar: dict, values: bool = False) -> dict:
    ...
    def fill_items_arr(parsed_meta: dict) -> None:
        try:
    ...
    def get_grid_items(variable: dict) -> list[dict]:
        children_array = []
        ...
        if fields:
            for field in fields:
                if variable.get('complex-grid'):
                    ...
                    if var_type in {'rating', 'singleChoice'}:
                ...
                if var_type not in {'ranking', 'rating', 'singleChoice'}:
    ...
    def get_main_info(
        ...
        def add_code_mapping(var_name: str, options: list) -> dict:
            encode = {
                var_name: {}
            }
            decode = {
        ...
        def get_options(
            ...
            if variable is None:
                return None
            col_values_arr = []
            def get_nodes_children(value: dict) -> None:
                node_obj = {}
                node_obj['text'] = {}
                try:
            ...
            for idx, value in enumerate(variable):
                if has_nodes:
                    ...
                    loop_reference = value.get('loopReference')
                    if(loop_reference and var_type == 'single'):
                        filtered_loop_ref = filter(lambda x: x['name']  == loop_reference, children_vars)
        ...
        variable = variable_meta.get('keys')[0] if is_child else variable_meta

        forsta_var_type = variable.get('variableType')

        options = variable.get("nodes") if has_nodes else variable.get("options")
        ...
        if forsta_var_type != 'rating' and not is_single_grid_var:
            variable_obj['parent'] = {}
        if var_type not in {'float', 'int', 'single'} and forsta_var_type != 'rating' and not is_single_grid_var:
        ...
        if var_type == 'array':
            ...
            if forsta_var_type in {'rating', 'singleChoice'}:
        ...
        if var_type not in {'float', 'int', 'array', 'string', 'date'}:
    ...
    def reformat_loop_data(loop_var: str, loop_of_loop: str | None = None) -> None:
                ...
                for idx, value in enumerate(old_values):
                    for k, v in value.items():
                        ...
                        if loop_of_loop is None:
                            if new_key != loop_var:
                                new_key = f'{loop_var}_{new_key}'
                        elif new_key != loop_of_loop:
                            new_key = f'{loop_var}_{new_key}'
                        else:
                            new_key = loop_var
                        new_key = f'{new_key}_{str(idx + 1)}'
                        data[new_key] = v
            except KeyError:
                pass

    def parse_forsta_types(variable: dict, loop_of_loop: str | None = None) -> None:
        forsta_var_type = variable.get('variableType')
        ...
        if has_parent:
            if has_parent in multigrid_vars:
                ...
                try:
                    ...
                    filtered_parent = {}
                if filtered_parent.get('variableType') == 'multiGrid' and has_parent not in multigrid_vars:
                    try:
                        language_code = variable.get('texts')[0].get('languageId')
                    except TypeError:
                        language_code = None
                ...
                if filtered_parent.get('variableType') == 'grid3D' and has_parent not in grid3d_vars:
                    try:
                        language_code = variable.get('titles')[0].get('languageId')
                    except TypeError:
                        language_code = None
        ...
        if forsta_var_type == 'multiGrid' and variable['name'] not in multigrid_vars:
    ...
    def set_as_loop(variable: dict) -> dict:
    ...
    if isinstance(data_json, list):
        ...
        with open(data_json) as read_data_file:
            data_parsed = json.load(read_data_file)

    if isinstance(meta_json, dict):
        meta_parsed = meta_json
    else:
        with open(meta_json) as read_meta_file:
    ...
    for _k, v in multigrid_vars.items():
    ...
    for var in to_qp_format.keys():
        var_mapping = to_qp_format.get(var)
        df[var] = df[var].apply(lambda x: var_mapping.get(x))
