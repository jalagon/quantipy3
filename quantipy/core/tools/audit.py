...
"""DataSet comparison and audit utilities for quantipy survey data.

This module provides the Audit class for comparing multiple DataSet instances,
identifying mismatches in variable names, types, labels, and categories across
...
class Audit(object):
    ...
    def __getitem__(self, ds):
        ...
        if isinstance(ds[0], str):
            not_incl = [
                d for d in ds if d not in self.ds_names +
                list(
    ...
    def __contains__(self, ds):
        for d in ds:
            if d not in list(self.ds_alias.keys()) or d in list(self.ds_alias.values()):
    ...
    def rename_by(self, name, datasets='all', ignore=[]):
        ...
        if not datasets:
            ...
            datasets = [
                alias for alias in list(
                    self.ds_alias.values()) if not alias == name]
        else:
