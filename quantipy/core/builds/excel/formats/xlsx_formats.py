"""Excel format specifications for quantipy3 reports.

This module provides the XlsxFormats class for generating comprehensive Excel
format dictionaries used in quantipy Excel report generation.
"""
...
DEFAULT = ["DEFAULT", "bg-DEFAULT", "frow-bg-DEFAULT"]
...
for i, name in enumerate(N_PCT[:]):
    N_PCT[i + 1 : i + 1] = [f"{name}-UP"]
    N_PCT[i + 2 : i + 2] = [f"{name}-DOWN"]
...
class XlsxFormats:
    """Excel format generator for quantipy reports.

    This class creates comprehensive Excel format dictionaries used by ExcelPainter
    to generate professional-looking Excel reports with proper styling, borders,
    colors, and typography for statistical data presentation.

    The class supports customizable formatting properties and generates format
    objects for different cell types including data cells, headers, statistical
    tests, and descriptive statistics.
    """

    def __init__(self, properties: dict[str, Any] | None = None) -> None:
        """Initialize Excel format generator with custom properties.

        Parameters
        ----------
        properties : dict[str, Any] | None, default None
            Dictionary of custom formatting properties to override defaults.
            Keys should match setter method names (without 'set_' prefix).

        Notes
        -----
        The constructor sets up default values for positioning, image handling,
        colors, fonts, borders, and other formatting specifications that can
        be customized through the properties parameter.
        """
        ...
        self.format_dict.update(
            {
                ...
                "TOC-10": {"font_name": self.font_name, "font_size": 10},
                "TOC-url": {
                    "font_name": self.font_name,
                    "font_size": 10,
                    "font_color": "blue",
    ...
    def _add_left(self):
        ...
        self._update_format_dict(
            border="left", additional_borders=["right", "top", "bottom"]
        )
    ...
    def _add_right(self):
        ...
        """
        self._update_format_dict(border="right", additional_borders=["top", "bottom"])
    ...
    def _add_interior(self):
        ...
        """
        self._update_format_dict(
            border="interior", additional_borders=["top", "bottom"]
        )
    ...
    def _update_format_dict(self, border, additional_borders):
        ...
        borders_list.extend(
            [
                "-{}-".format("-".join(x)) if isinstance(x, tuple) else f"-{x}-"
                for x in additional_borders
            ]
        )
    ...
    def _get_value(self, key):
        ...
        for border in ["left", "right", "top", "bottom"]:
            if f"{border}-" in key:
                ...
                if any(conditions):
                    ...
                    result.update(
                        self._get_border(
                            border, self.border_style_ext, self.border_color_nets_top
                        )
                    )
        ...
        if "left" not in key and not key.endswith("-STR"):
        ...
        if key.endswith(("-DESCRIPTIVES", "-DESCRIPTIVES-XT")):
            if not key.endswith("XT"):
                for border in ["top", "bottom"]:
                    if border not in list(result.keys()):
                        result.update(
                            self._get_border(
                                border,
            ...
            if "bg" in key:
                result.update(self._get_bg_format("DESCRIPTIVES", True))
            ...
            for border in ["top"]:
                if border not in list(result.keys()):
                    result.update(
                        self._get_border(
                            border, self.border_style_int, self.border_color_nets_top
                        )
                    )
            result.update(self._get_num_format("N"))
            ...
            if "right" not in list(result.keys()):
        ...
        # Add top row if "frow"
        if "frow" in key and "top" not in key:
            if key.endswith("DESCRIPTIVES"):
                result.update(
                    self._get_border(
                        "top", self.border_style_int, self.border_color_descriptives_top
                    )
                )
                result = {k: v for k, v in list(result.items()) if "bottom" not in k}
            elif key.endswith(("NET", "NET-UP", "NET-DOWN")):
                result.update(
                    self._get_border(
                        "top", self.border_style_int, self.border_color_nets_top
                    )
                )
            else:
        ...
        if "brow" in key:
            result = {k: v for k, v in list(result.items()) if not k.startswith("top")}
    ...
    def _get_border(self, border, border_style, border_color=None):
        ...
        result = {border: border_style, f"{border}_color": border_color}
        return result
    ...
    def _get_font_format(self, cell):
        ...
        if cell in ["DEFAULT", "N", "PCT"]:
            result = {"font_name": self.font_name, "font_size": self.font_size}
        elif cell == "BASE":
            result = {
                "font_name": self.font_name,
                "font_size": self.font_size,
    ...
    def _get_bg_format(self, cell, required):
        ...
        if cell in ["DEFAULT", "N", "PCT", "DESCRIPTIVES"]:
            ...
            result = {"bg_color": self.bg_color_nets if required else "#FFFFFF"}
        elif cell == "TESTS":
            result = {"bg_color": self.bg_color_tests if required else "#FFFFFF"}
        return result
