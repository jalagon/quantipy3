...
"""PowerPoint visual editor for quantipy3.

This module provides visual editing capabilities for PowerPoint presentations,
including slide layout management, shape copying, chart replication, and
comprehensive formatting control for quantipy reports.
...
def rename_duplicate_shape_names(pres_path, overwrite=True):
    ...
    if overwrite:
        prs.save(f'{file_path}\\{file_name}.pptx')
    else:
        prs.save(f'{file_path}\\{file_name}_edited.pptx')

'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'

def read_pptx(pres_path, slide_num=None, save_as_json=True):
    '''
...
def read_chart_properties(pres_path, slide_num, chart_name):
    ...
    for i, sld in enumerate(prs.slides, start=1):
        if i == slide_num:
            for _x, shp in enumerate(sld.shapes):

                if shp.name == chart_name:

                    print('chart >\n')
                    print(f'   chart_style: {shp.chart.chart_style}')
                    print(f'   has_legend: {shp.chart.has_legend}')
                    print(f'   legend: {shp.chart.legend}\n')
                    print('-'*110)

                    caxis = shp.chart.category_axis

                    print('chart > category axis properties\n')
                    print(f'   has_major_gridlines: {caxis.has_major_gridlines}')
                    print(f'   has_minor_gridlines: {caxis.has_minor_gridlines}')
                    print(f'   major_tick_mark: {caxis.major_tick_mark}')
                    print(f'   maximum_scale: {caxis.maximum_scale}')
                    print(f'   minimum_scale: {caxis.minimum_scale}')
                    print(f'   minor_tick_mark: {caxis.minor_tick_mark}')
                    print(f'   tick_labels: {str(caxis.tick_labels)}')
                    print(f'   tick_label_position: {caxis.tick_label_position}')
                    print(f'   tick_labels_font_name: {caxis.tick_labels.font.name}')
                    print(f'   tick_labels_font_size: {caxis.tick_labels.font.size}')
                    print(f'   tick_labels_font_bold: {caxis.tick_labels.font.bold}')
                    print(f'   tick_labels_font_color: {caxis.tick_labels.font.color}')
                    print(f'   tick_labels_font_italic: {caxis.tick_labels.font.italic}')
                    print(f'   tick_labels_font_underline: {caxis.tick_labels.font.underline}')
                    print(f'   tick_labels_number_format: {caxis.tick_labels.number_format}')
                    print(f'   tick_labels_number_format_is_linked: {caxis.tick_labels.number_format_is_linked}')
                    print(f'   tick_labels_offset: {caxis.tick_labels.offset}')
                    print(f'   visible: {caxis.visible}\n')
                    print('-'*110)

                    vaxis = shp.chart.value_axis

                    print('chart > value axis properties\n')
                    print(f'   has_major_gridlines: {vaxis.has_major_gridlines}')
                    print(f'   has_minor_gridlines: {vaxis.has_minor_gridlines}')
                    print(f'   major_tick_mark: {vaxis.major_tick_mark}')
                    print(f'   maximum_scale: {vaxis.maximum_scale}')
                    print(f'   minimum_scale: {vaxis.minimum_scale}')
                    print(f'   major_unit: {vaxis.major_unit}')
                    print(f'   minor_unit: {vaxis.minor_unit}')
                    print(f'   minor_tick_mark: {vaxis.minor_tick_mark}')
                    print(f'   tick_labels: {vaxis.tick_labels}')
                    print(f'   tick_label_position: {vaxis.tick_label_position}')
                    print(f'   tick_labels_font_name: {vaxis.tick_labels.font.name}')
                    print(f'   tick_labels_font_size: {vaxis.tick_labels.font.size}')
                    print(f'   tick_labels_font_bold: {vaxis.tick_labels.font.bold}')
                    print(f'   tick_labels_font_color: {vaxis.tick_labels.font.color}')
                    print(f'   tick_labels_font_italic: {vaxis.tick_labels.font.italic}')
                    print(f'   tick_labels_font_underline: {vaxis.tick_labels.font.underline}')
                    print(f'   tick_labels_font: {vaxis.tick_labels.font}')
                    print(f'   tick_labels_number_format: {vaxis.tick_labels.number_format}')
                    print(f'   tick_labels_number_format_is_linked: {vaxis.tick_labels.number_format_is_linked}')
                    print(f'   visible: {vaxis.visible}\n')
                    print('-'*110)

                    for item in shp.chart.plots:

                        print('chart > plot\n')
                        print(f'   plot_categories: {item.categories}')
                        print(f'   plot_gap_width: {item.gap_width}')
                        print(f'   has_data_labels: {item.has_data_labels}')
                        print(f'   overlap: {item.overlap}')
                        print(f'   vary_by_categories: {item.vary_by_categories}\n')
                        print('-'*110)
                        ...
                        print(f'   data_label_font_name: {font.name}')
                        print(f'   data_label_font_size: {font.size}')
                        print(f'   data_label_font_bold: {font.bold}')
                        print(f'   data_label_font_color {font.color}')
                        print(f'   data_label_font_fill {font.fill}')
                        print(f'   data_label_font_italic: {font.italic}')
                        print(f'   data_label_font_underline: {font.underline}\n')
                        print('-'*110)

                        for ser in item.series:

                            print('chart > plot > series\n')
                            print(f'   series_fill_type: {ser.fill.type}')
                            print(f'   series_invert_if_neg: {ser.invert_if_negative}')
                            print(f'   series_line: {ser.line}')
                            print(f'   series_name: {ser.name}')
                            print(f'   series_values: {ser.values}')
...
def replace_chart_data_in_prs(pres_path, slide_num, chart_name, df):
    ...
    prs.save(f'{PRES_FOLDER_FOLDER}\\{PRES_NAME}_edited.pptx')
...
def get_slide_layout_names(pptx):
    ...
    for _i, slide_layout in enumerate(pptx.slide_layouts):
...
def return_slide_layout_by_name(pptx, slide_layout_name):
    ...
    for slide_layout in pptx.slide_layouts:
        ...
        raise Exception(
            f'Slide layout: {slide_layout_name} not found\n')
...
def copy_chart_properties(shp_copy_from, sld_rep):
    ...
    str(shp_copy_from.chart.chart_type).split(" ")[0]
