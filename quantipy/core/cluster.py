"""Cluster module for quantipy data processing.
...
class Cluster(OrderedDict):
    ...
    def _verify_banked_chain_spec(self, spec: dict[str, Any]) -> bool:
        ...
        try:
            ...
            cbases = spec['bases']
            for c in citems:
                # Validate structure but don't store unused variables
                _ = c['chain']
                _ = c['text']
    ...
    def bank_chains(self, spec: dict[str, Any], text_key: str) -> None:
        ...
        bchain.views = [vk_test for vk_test in bchain.views if 'cbase' in vk_test]
            ...
            if banked[yk].columns.levels[1][0] == '@':
                banked[yk] = pd.DataFrame(
                    ...
                    columns=pd.MultiIndex.from_tuples(
                        [(spec['name'], '@')], names=['Question', 'Values']
                    ),
                )
